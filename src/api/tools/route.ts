...
import { validateToolRequest, sanitizeInput, generateSessionId } from '../types';
import { toolRateLimiter, getClientIdentifier, createRateLimitResponse } from '../rate-limiter';
  ...
  return new Response(
    JSON.stringify({
      ...
      usage: {
        ...
        searchDocs: {
          method: 'POST',
          body: { tool: 'searchDocs', input: { query: 'string', maxResults: 'number?' } },
        },
      },
...
export async function POST(request: Request): Promise<Response> {
  ...
  switch (toolRequest.tool) {
    case 'analyzeError':
      if (!toolRequest.input.errorLog) {
        return createErrorResponse('errorLog is required', 'MISSING_INPUT', 400);
      }
  ...
  try {
    ...
    const workerResponse = await forwardToWorkerTool(toolRequest.tool, toolInput);
