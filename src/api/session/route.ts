...
function getSessionId(request: Request): string | null {
  ...
  if (cookieHeader) {
    ...
    const sessionCookie = cookies.find((c) =>
      c.startsWith("devcopilot_session=")
    );
    if (sessionCookie) {
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    const workerResponse = await forwardToWorkerSession(
      "POST",
      sessionId,
      body
    );
    ...
    if (isNewSession) {
      headers["Set-Cookie"] =
        `devcopilot_session=${sessionId}; Path=/; HttpOnly; SameSite=Strict; Max-Age=86400`;
    }
    ...
    return new Response(
      ...
      }),
      {
        ...
        headers: {
          "Content-Type": "application/json",
          "Set-Cookie":
            "devcopilot_session=; Path=/; HttpOnly; SameSite=Strict; Max-Age=0",
          ...CORS_HEADERS
        }
      }
    );
