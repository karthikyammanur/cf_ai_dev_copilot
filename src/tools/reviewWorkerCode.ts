 ...
 */
function extractSnippet(lines: string[], lineNum: number, context: number = 1): string {
...
function _isInString(code: string, position: number): boolean {
  ...
  return (singleQuotes % 2 !== 0) || (doubleQuotes % 2 !== 0) || (templateLiterals % 2 !== 0);
}
...
const ANALYSIS_PATTERNS: AnalysisPattern[] = [
  ...
  // =========================================================================
  {
    ...
    issue: 'Using JSON.parse() on potentially large response text can block the CPU and exhaust time limits.',
    recommendation: 'Use response.json() instead which is optimized for streaming, or implement streaming JSON parsing for very large payloads.',
    fixedCode: `// ❌ Before
...
import { JSONParser } from '@streamparser/json';`
...

  {
    ...
    issue: 'Infinite or very large loops can exhaust CPU time limits (10ms free, 30ms paid, 30s Unbound).',
    recommendation: 'Add loop limits, use async iteration with breaks, or offload to Durable Objects/Queues for long-running processes.',
    fixedCode: `// ❌ Before
while (true) {
  // process forever
}
...
while (iterations < MAX_ITERATIONS) {
...

  {
    ...
    issue: 'Awaiting inside a loop executes requests sequentially, wasting time when they could run in parallel.',
    recommendation: 'Use Promise.all() or Promise.allSettled() to parallelize independent async operations. Be mindful of subrequest limits (50 free, 1000 paid).',
...
const results = await Promise.allSettled(
...

  {
    ...
    issue: 'Buffering entire response body (.text(), .arrayBuffer(), .blob()) consumes memory and can hit the 128MB limit.',
    recommendation: 'Use streaming with TransformStream or pipe directly to response for large payloads.',
...
return new Response(readable);`
...

  {
    ...
    issue: 'External fetch calls without caching can lead to repeated slow requests and unnecessary subrequest usage.',
    recommendation: 'Use the Cache API to cache external responses at the edge, reducing latency and subrequest count.',
...
export default {
  ...
  // =========================================================================
  {
    ...
    issue: 'Hardcoded secrets in code can be exposed in version control, logs, and error messages.',
    recommendation: 'Store secrets using Wrangler secrets (wrangler secret put) and access via env bindings.',
...
export default {
...

  {
    ...
    issue: 'eval() and dynamic code execution can lead to code injection attacks if user input is included.',
    recommendation: 'Avoid eval() entirely. Use JSON.parse() for data, or pre-defined functions instead of dynamic code.',
...
const result = handlers[userInput]?.();`
...

  {
    ...
    severity: 'critical',
    patterns: [
      /text\/html['"\`]\s*\}\s*\)[^]*\$\{/g,  // HTML response with template literal
      /innerHTML\s*=/g,
      /document\.write\s*\(/g,
      /\.html\s*\([^)]*\+/g,  // jQuery .html() with concatenation
    ],
    issue: 'Directly inserting user input into HTML can lead to Cross-Site Scripting (XSS) attacks.',
    recommendation: 'Escape HTML entities before insertion, use textContent instead of innerHTML, or use a templating library with auto-escaping.',
...
return new Response(\`<div>\${escapeHtml(userInput)}</div>\`, {
...

  {
    ...
    issue: 'Redirecting to user-supplied URLs can lead to phishing attacks via open redirects.',
...
return Response.redirect(redirectUrl);`
  ...
  // =========================================================================
  {
    ...
    severity: 'warning',
    patterns: [
      /(?<!try\s*\{[^}]*)await\s+fetch\s*\(/g,
    ],
    issue: 'Fetch calls without try-catch can cause unhandled exceptions that crash the Worker.',
    recommendation: 'Wrap fetch calls in try-catch and handle network errors gracefully.',
...
try {
...

  {
    ...
    severity: 'warning',
    patterns: [
      /await\s+fetch\s*\([^)]+\)[^]*?\.json\s*\(\)/g,
      /await\s+fetch\s*\([^)]+\)[^]*?\.text\s*\(\)/g,
    ],
    skipIfPresent: [
      /\.ok\b/,
      /\.status\b/,
    ...
    issue: 'Reading response body without checking response.ok can lead to parsing errors on error responses.',
    recommendation: 'Always check response.ok or response.status before parsing the response body.',
...
const data = await response.json();`
...

  {
    ...
    issue: 'Empty catch blocks swallow errors silently, making debugging impossible.',
    recommendation: 'At minimum, log the error. Better yet, handle it appropriately or re-throw.',
...
try {
  ...
  // =========================================================================
  {
    ...
    issue: 'Background async operations after returning a response may be terminated before completion.',
    recommendation: 'Use ctx.waitUntil() to ensure background work completes even after the response is sent.',
...
export default {
...

  {
    ...
    issue: 'KV reads without caching options may not leverage edge caching effectively.',
    recommendation: 'Use cacheTtl option for frequently-read values to reduce KV read operations and latency.',
...
const config = await env.MY_KV.get('config', {
...

  {
    ...
    patterns: [
      ...
      /D1[^]*SELECT[^]*LIKE\s*['"]%/gi,  // Full-text search in D1
    ],
    issue: 'Storage choice may not be optimal for this use case. KV is best for key-value access, D1 for relational queries, R2 for large files.',
    recommendation: 'Consider: KV for config/sessions (<25MB values), D1 for relational data, R2 for files/blobs (>1MB), Vectorize for similarity search.',
...
await env.VECTOR_INDEX.query(embedding, { topK: 10 });`
...

  {
    ...
    issue: 'API responses without CORS headers will be blocked by browsers for cross-origin requests.',
    recommendation: 'Add CORS headers if this API is called from browsers on different domains.',
...
export default {
  ...
  // =========================================================================
  {
    ...
    issue: 'Excessive console logging can impact performance and clutter production logs.',
    recommendation: 'Use structured logging, log levels, and consider conditional logging based on environment.',
...
log('info', 'Request completed', { status: 200, duration: 45 });`
...

  {
    ...
    issue: 'Using "any" type or ignoring TypeScript checks reduces type safety and can hide bugs.',
    recommendation: 'Define proper types for your data structures. Use "unknown" instead of "any" when type is truly unknown.',
...
async function handleRequest(data: RequestData): Promise<ResponseData> {
...

  {
    ...
    recommendation: 'Extract magic numbers into named constants with clear meaning.',
...
export function reviewWorkerCodeFn(code: string): CodeReviewResult {
  ...
  if (findings.length === 0) {
    summary = 'No issues found. The code follows Cloudflare Workers best practices.';
  }
